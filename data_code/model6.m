...
% loop over all trials in low (=-1)/even (=0) and high(=1) decks and compute choice probabilities  
 for j=1:180 %run over all 180 trials
  if (data.C(j)==1||2) % exclude trials with no choice (data(k).C=Nan)
   ...
   if data.C(j)==1
     if data.O(j)==1  
        delta_pos = data.O(j) - Q(:,data.D(j)+2) - data.N(j); %%% adjusted Q-learning
        delta_neg = zeros(S,1);
        delta_neutral=zeros(S,1);  
     elseif data.O(j)==-1
        delta_neg = data.O(j) - Q(:,data.D(j)+2) - data.N(j); %%% adjusted Q-learning
        delta_pos = zeros(S,1);
        delta_neutral=zeros(S,1);
     else
        delta_neutral=data.O(j) - Q(:,data.D(j)+2) - data.N(j); %%% adjusted Q-learning
        delta_pos = zeros(S,1);
        delta_neg = zeros(S,1);           
     end
