function [latents] = model6_sim(P, EXPT)
...
for j=1:180 %run over all 180 trials
    if mod(j,15)==1 %% main task
        ...
        if latents.sim_C(j)==1
            if latents.O(j)==1
                delta_pos = latents.O(j) - Q(:,latents.D(j)) - latents.N_norm(j); %%% adjusted Q-learning (JM)
                delta_neg = zeros(S,1);
                delta_neutral=zeros(S,1);  
            elseif latents.O(j)==-1
                delta_neg = latents.O(j) - Q(:,latents.D(j)) - latents.N_norm(j); %%% adjusted Q-learning (JM)
                delta_pos = zeros(S,1);
                delta_neutral=zeros(S,1);
            else
                delta_neutral=latents.O(j) - Q(:,latents.D(j)) - latents.N_norm(j); %%% adjusted Q-learning (JM)
                delta_pos = zeros(S,1);
                delta_neg = zeros(S,1);
            end  
